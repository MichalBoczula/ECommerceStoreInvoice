...
- Domain and Application each retain a separate 70% line-coverage floor; Infrastructure coverage is reported without a minimum. Coverage alone never substitutes for response-path and persistence assertions. Normal compiler warnings remain visible without automatically failing CI.
